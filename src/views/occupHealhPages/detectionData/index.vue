  ...
  <div class="viewBody">
    ...
    <div class="main-content" ref="maincontent">
      ...
      <el-table ref="multipleTable" :data="tableData.listItem" :border="true" style="width: 100%" v-loading="tableloading" :height="Height">
        ...
        <el-table-column prop="noise" label="噪声" :align="'center'"></el-table-column>
    ...
    <el-dialog :visible.sync="addDialogVisible" width="30%">
      ...
      <el-form :model="addOrUpdateReq" status-icon ref="addApiForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="噪音" :prop="'noise'" :rules="[{ required: true, message: '请填写噪音' }]">
          <el-input v-model="addOrUpdateReq.noise"></el-input>
        </el-form-item>
        <el-form-item label="温度" :prop="'temperature'" :rules="[{ required: true, message: '请填写温度' }]">
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.type === "add") {
            occupHealth
              .setTestDataList(this.addOrUpdateReq)
              .then((res) => {
            ...
            occupHealth
              .editTestDataList({ ...this.addOrUpdateReq })
              .then((res) => {
                if (res.data.code === 200) {
